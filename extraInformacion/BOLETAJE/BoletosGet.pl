...
sub getFechaTerminal{
   ...
   my $query = "SELECT DISTINCT(FECHA_BOLETO) FROM PDV_T_BOLETO  ".
	       "WHERE FECHA_BOLETO BETWEEN '2019-01-01' AND (SELECT DATE_SUB(CURRENT_DATE(),INTERVAL 1 DAY)) AND  ".
...
sub setUpdateMovWeb{
	...
	$query = "UPDATE $tabla_anual SET HORA_CORRIDA = ? ".
		 "WHERE ID_BOLETO = ? AND ID_TERMINAL = ? AND TRAB_ID = ?";
	my $stt = $_SERVER->prepare( $query );
        $stt->bind_param(1, $a_hora[0]);
        $stt->bind_param(2, $a_anual[0]);
	$stt->bind_param(3, $a_anual[1]);
	$stt->bind_param(4, $a_anual[2]);
	$stt->execute();

	$query = "UPDATE PDV_T_BOLETO SET HORA_CORRIDA = ? ".
		 "WHERE ID_BOLETO = ? AND ID_TERMINAL = ? AND TRAB_ID = ?";
