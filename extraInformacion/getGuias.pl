...
sub getFechaTerminal{
   ...
   while(my @kk = $stv->fetchrow_array()){
        $query = "SELECT COUNT(*) total ".
                 ...
                 "WHERE FECHA = ? AND ID_TERMINAL = ? AND TABLA = 'PDV_T_GUIAS' ";
   ...
   $query = "SELECT FECHA ".
	    "FROM ADM_STATUS_TABLA ".
	    "WHERE ID_TERMINAL = ? AND TABLA = 'PDV_T_GUIA' AND STATUS = 0 ". 
	    "ORDER BY 1 desc ".
	    "LIMIT 1";
...
sub actualizaADM{
    ...
    my $query = "UPDATE ADM_STATUS_TABLA SET STATUS = ?, LUGAR  = ?, CORPORATIVO = ?  WHERE FECHA = ? AND TABLA = 'PDV_T_GUIA' ".
                "AND ID_TERMINAL = ?";
    my $sth = $_SERVER->prepare( $query ); 
...
sub getGuia{
    my ($fecha, $terminal) = @_;
    my $query = "SELECT ID_GUIA, ID_TERMINAL, FECHA, ID_CORRIDA, ID_DESTINO, NO_BUS,INGRESO, ID_OPERADOR, ".
		"TRAB_ID, TIPOSERVICIO, ID_RUTA, FECHA_HORA,OLLIN_FOLIO, OLLIN_TIPO_CUOTA, OLLIN_PROVEEDOR, ".
		"TIPO_SERVICIO_CEMO, FOLIO_CEMO, BOLETERA_FOLIO ".
		"FROM PDV_T_GUIA ".
		"WHERE FECHA = ? AND ID_TERMINAL = ? ";
	...
	$query = "INSERT INTO PDV_T_GUIA(ID_GUIA, ID_TERMINAL, FECHA, ID_CORRIDA, ID_DESTINO, NO_BUS, INGRESO, ID_OPERADOR, ".
	         "TRAB_ID, TIPOSERVICIO, ID_RUTA, FECHA_HORA, OLLIN_FOLIO, OLLIN_TIPO_CUOTA, OLLIN_PROVEEDOR, ".
	         "TIPO_SERVICIO_CEMO, FOLIO_CEMO, BOLETERA_FOLIO) ".
	         "VALUES($outs[0], '$outs[1]', '$outs[2]', '$outs[3]', '$outs[4]', $outs[5], $outs[6], '$outs[7]', '$outs[8]' ".
		 "$outs[9], $outs[10], '$outs[11]', $outs[12], $outs[13], $outs[14], '$outs[15]', '$outs[16]', $outs[17] )";
	eval{
