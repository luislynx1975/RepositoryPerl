...
sub getTerminales{
   ...
   my $query = "SELECT ID_TERMINAL, BD_USUARIO, BD_PASSWORD,IPV4 ".
	       "FROM PDV_C_TERMINAL ".
#               "WHERE ESTATUS = 'A' AND ID_TERMINAL  in ('JOJU') ORDER BY 1";
               "WHERE ESTATUS = 'A' AND ID_TERMINAL not in ('WEB','SD2','SD1','MOV') ORDER BY 1";
...
sub getFechaTerminal{
   ...
   while(my @kk = $stv->fetchrow_array()){
        $query = "SELECT COUNT(*) total ".
                 ...
                 "WHERE FECHA = ? AND ID_TERMINAL = ? AND TABLA = 'PDV_T_CORRIDA_D' ";
   ...
   $query = "SELECT FECHA ".
	    "FROM ADM_STATUS_TABLA ".
	    "WHERE ID_TERMINAL = ? AND TABLA = 'PDV_T_CORRIDA_D' AND STATUS = 0 ". 
	    "ORDER BY 1 desc ".
	    "LIMIT 1";
...
sub getTotalCorridaDCorporativo{
   my ($fecha, $terminal) = @_;
   my $query = "SELECT COUNT(*) AS TOTAL ".
	       "FROM PDV_T_CORRIDA_D ".
               "WHERE FECHA = ? AND ID_TERMINAL = ?";
...
sub actualizaADM{
    my ($fecha, $ter) = @_;
    my $tt_corpo = &getTotalCorridaDCorporativo($fecha, $ter);
    my $tt_lugar = &getTotalCorridaDREMOTO($fecha, $ter);
    my $estado = 0;
    ...
    my $query = "UPDATE ADM_STATUS_TABLA SET STATUS = ?, LUGAR  = ?, CORPORATIVO = ?  WHERE FECHA = ? AND TABLA = 'PDV_T_CORRIDA_D' ".
                "AND ID_TERMINAL = ?";
    my $sth = $_SERVER->prepare( $query ); 
...
sub getCorridaD{
    my ($fecha, $terminal) = @_;
print "$fecha $terminal \n";
    my $query = "SELECT ID_CORRIDA, FECHA, ID_TERMINAL, HORA, CUPO, PIE, ESTATUS, ID_RUTA, TRAB_ID ".
		"FROM PDV_T_CORRIDA_D ".
		"WHERE FECHA = ? AND ID_TERMINAL = ? ";
	...
	$query = "INSERT INTO PDV_T_CORRIDA_D(ID_CORRIDA, FECHA, ID_TERMINAL, HORA, CUPO, PIE, ESTATUS, ID_RUTA, TRAB_ID) ".
...
#print "aqui voy  $tabla[3]";
#        if(  $p->ping($tabla[3])){
	   $_REMOTO = &conectaServer($db, $tabla[3], $tabla[1], $tabla[2]);
      	   if(defined $_REMOTO){
              &getCorridaD($aFechaNew, $tabla[0]);
