...
sub getFechaTerminal{
   ...
   my $idx = 0;
   my @outs = undef;
   my $query = "SELECT DISTINCT(CAST(G.FECHA_HORA AS DATE)) ".
	       "FROM PDV_T_GUIA_DESPACHO G ".
               "WHERE CAST(G.FECHA_HORA AS DATE) BETWEEN '2020-01-01' AND (SELECT DATE_SUB(CURRENT_DATE(),INTERVAL 1 DAY)) AND ".
               "G.OCULTO IS NOT NULL";
   ...
   $query = "SELECT FECHA ".
	    ...
	    "WHERE ID_TERMINAL = ? AND TABLA = 'PDV_T_GUIA_DESPACHO' AND STATUS = 0 ". 
	    "ORDER BY 1 desc ".
	    "LIMIT 1";
...
sub actualizaADM{
    ...
    my $query = "UPDATE ADM_STATUS_TABLA SET STATUS = ?, LUGAR  = ?, CORPORATIVO = ?  WHERE FECHA = ? AND TABLA = 'PDV_T_GUIA_DESPACHO' ".
                "AND ID_TERMINAL = ?";
    my $sth = $_SERVER->prepare( $query ); 
...
sub getGuiaDespacho{
    my ($fecha, $terminal) = @_;
    my $query = "SELECT  ID_GUIA, ID_TERMINAL, DESTINO, ID_DESPACHO, PAGO, ID_CORRIDA, OCULTO, FECHA_HORA, WEB_SERVICE ".
	       "FROM PDV_T_GUIA_DESPACHO G ".
               "WHERE CAST(G.FECHA_HORA AS DATE) = ? ";
    my $stv = $_REMOTO->prepare( $query );
    $stv->bind_param(1, $fecha);
    $stv->execute();

    while(my @outs = $stv->fetchrow_array()){
	    $query = "INSERT INTO PDV_T_GUIA_DESPACHO(ID_GUIA, ID_TERMINAL, DESTINO, ID_DESPACHO, PAGO, ID_CORRIDA, OCULTO, FECHA_HORA, WEB_SERVICE ) ".
