...
###################Insertamos en la tabla de translado
	...
	$db_factura->execute();
	#####Aactualizamos la tabla de boleto con el comprobante y tipo_factura = G
	my $sql = "UPDATE $tableNombre SET COMPROBANTE = ?, TIPO_FACTURA = ?, FACTURADO = 1,  COMPLEMENTO_2 = 'FACTURADO MES JUNIO'  ".
...
sub getStatusGlobal{
    ...
    my $sql = "SELECT  DISTINCT(e.ID_TERMINAL), e.TIPOSERVICIO, COUNT(*) ".
	      "FROM $tableNombre e ". 
	      "WHERE e.COMPLEMENTO_2 = 'JUNIO'  AND TARIFA > 0 AND ESTATUS = 'V' ".
	      "GROUP by 2, 1  ".
	      "UNION  ".
	      "SELECT  DISTINCT(e.ID_TERMINAL), e.TIPOSERVICIO, COUNT(*) ".
	      "FROM $tableNombre e  ". 
	      "WHERE e.COMPLEMENTO_2 = 'JUNIO'  AND e.ID_TERMINAL IN ('WEB','MOV') AND TARIFA > 0 AND ESTATUS = 'V' ".
	      "GROUP by 2, 1 ".
	      "ORDER BY 1"; 
...
sub getBoletoServicio5{
    ...
    if($datos[5] != 5){
       $sql = "SELECT COUNT(*) ".
	      ...
	      "AND e.TIPOSERVICIO = ? AND id_terminal = ? AND e.COMPLEMENTO_2 = 'JUNIO' AND TARIFA > 0";
       $db_boleto = $S_BOLETO->prepare( $sql );
    ...
    if($datos[5] == 5){
	  ...
	  $iva = 16;
       ...
       $sql = "SELECT COUNT(*) ".
	      "FROM $tableNombre e ".
	      "WHERE  e.FECHA_HORA_BOLETO BETWEEN ? AND ?  ".
	      "AND e.TIPOSERVICIO = ? AND id_terminal = ? AND e.IVA = ? AND e.COMPLEMENTO_2 = 'JUNIO' AND TARIFA > 0 ";
       $db_boleto = $S_BOLETO->prepare( $sql );
       $db_boleto->bind_param(1, $datos[1]);
       $db_boleto->bind_param(2, $datos[2]);
       $db_boleto->bind_param(3, $datos[5]);
...
#AND e.TIPOSERVICIO = 2 AND id_terminal = 'MEX' AND e.COMPLEMENTO_2 = 'JUNIO' 
####################################################################
sub insertTmpBoleto7000{
    ...
    $sql = "INSERT INTO TMP_BOLETO ".
	   "SELECT * ".
	   "FROM $tableNombre e  ".
	   "WHERE e.TIPOSERVICIO = ? AND id_terminal = ? AND e.COMPLEMENTO_2 = 'JUNIO' AND ESTATUS = 'V' ".
	   "LIMIT 7000";
    $db_corpo = $S_BOLETO->prepare( $sql );
...
sub cambiaMesComplemento{# año terminal servicio extraordinario
    ...
    my $sql = "UPDATE $tableNombre e INNER JOIN TMP_BOLETO t ON t.ID_BOLETO = e.ID_BOLETO AND t.ID_TERMINAL = e.ID_TERMINAL AND t.TRAB_ID = e.TRAB_ID ".
	      "SET e.COMPLEMENTO_2 = 'FACTURADO MES JUNIO'";
    my $db_corpo = $S_BOLETO->prepare( $sql );
    $db_corpo->execute();
}
