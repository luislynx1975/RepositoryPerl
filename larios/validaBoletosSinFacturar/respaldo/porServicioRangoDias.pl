...
sub getStatusGlobal{
    ...
    my $sql = "SELECT  DISTINCT(e.ID_TERMINAL), e.TIPOSERVICIO, COUNT(*) ".
	      "FROM ENERO_2022 e  INNER JOIN PDV_C_GRUPO_SERVICIOS g ON e.ID_TERMINAL = g.ID_TERMINAL ".
	      "WHERE e.COMPLEMENTO_2 = 'MAYO'  AND g.ID_GRUPO = 'PUL' AND TARIFA > 0 AND ESTATUS = 'V' ".
	      "GROUP by 2, 1  ".
	      "UNION  ".
	      "SELECT  DISTINCT(e.ID_TERMINAL), e.TIPOSERVICIO, COUNT(*) ".
	      "FROM ENERO_2022 e  ". 
	      "WHERE e.COMPLEMENTO_2 = 'MAYO'  AND e.ID_TERMINAL IN ('WEB','MOV') AND TARIFA > 0 AND ESTATUS = 'V' ".
	      "GROUP by 2, 1 ".
	      "ORDER BY 1"; 
...
sub getBoletoServicio5{
    ...
    if($datos[5] != 5){
       $sql = "SELECT COUNT(*) ".
	      ...
	      "AND e.TIPOSERVICIO = ? AND id_terminal = ? AND e.COMPLEMENTO_2 = 'MAYO' AND TARIFA > 0";
       $db_boleto = $S_BOLETO->prepare( $sql );
    ...
    if($datos[5] == 5){
	  ...
	  $iva = 16;
       ...
       $sql = "SELECT COUNT(*) ".
	      "FROM $tableNombre e ".
	      "WHERE  e.FECHA_HORA_BOLETO BETWEEN ? AND ?  ".
	      "AND e.TIPOSERVICIO = ? AND id_terminal = ? AND e.IVA = ? AND e.COMPLEMENTO_2 = 'MAYO' AND TARIFA > 0 ";
       $db_boleto = $S_BOLETO->prepare( $sql );
       $db_boleto->bind_param(1, $datos[1]);
       $db_boleto->bind_param(2, $datos[2]);
       $db_boleto->bind_param(3, $datos[5]);
...
sub getGlobalMensual{
    ...
    my $idx = 0;
    my @out = ();
    my $sql = "SELECT TERMINAL, INICIO, FIN, SERVICIO, BOLETOS_TOTAL, EXTRAORDINARIO ".
	      "FROM FAC_T_GLOBAL_MENSUAL ".
...
#AND e.TIPOSERVICIO = 2 AND id_terminal = 'MEX' AND e.COMPLEMENTO_2 = 'MAYO' 
####################################################################
sub insertTmpBoleto7000{
    ...
    $sql = "INSERT INTO TMP_BOLETO ".
	   "SELECT * ".
	   "FROM ENERO_2022 e  ".
	   "WHERE e.TIPOSERVICIO = ? AND id_terminal = ? AND e.COMPLEMENTO_2 = 'MAYO' AND ESTATUS = 'V' ".
	   "LIMIT 7000";
    $db_corpo = $S_BOLETO->prepare( $sql );
...
sub cambiaMesComplemento{# año terminal servicio extraordinario
    ...
    my $sql = "UPDATE $tableNombre e INNER JOIN TMP_BOLETO t ON t.ID_BOLETO = e.ID_BOLETO AND t.ID_TERMINAL = e.ID_TERMINAL AND t.TRAB_ID = e.TRAB_ID ".
	      "SET e.COMPLEMENTO_2 = 'FACTURADO MES'";
    my $db_corpo = $S_BOLETO->prepare( $sql );
    $db_corpo->execute();
=pod
    if($datos[2] != 5){
	$sql = "UPDATE $tableNombre SET COMPLEMENTO_2 = 'FACTURADO MES' ".
	       "WHERE ID_TERMINAL = ? AND TIPOSERVICIO = ? AND COMPLEMENTO_2 = 'MAYO'";
	my $db_servi = $S_BOLETO->prepare( $sql );
        $db_servi->bind_param(1, $datos[1] );
        $db_servi->bind_param(2, $datos[2] );
        $db_servi->execute();
    }
    if($datos[2] == 5){
	if($datos[3] == 0){
	...
	$sql = "UPDATE $tableNombre SET COMPLEMENTO_2 = 'FACTURADO MES' ".
	       "WHERE ID_TERMINAL = ? AND TIPOSERVICIO = ? AND iva = ? AND COMPLEMENTO_2 = 'MAYO'";	
	my $db_servi = $S_BOLETO->prepare( $sql );
        $db_servi->bind_param(1, $datos[1] );
        $db_servi->bind_param(2, $datos[2] );
        $db_servi->bind_param(3, $iva );
        $db_servi->execute();
    } 
=cut
}
...
for my $idx (0..@global -1){
   if($global[$idx][4] > $NUM_ROWS ){
	...
	&insertTmpBoleto7000($global[$idx][1] . ' 00:00:00', $global[$idx][2] . ' 23:59:59', $global[$idx][3], $global[$idx][0]);
