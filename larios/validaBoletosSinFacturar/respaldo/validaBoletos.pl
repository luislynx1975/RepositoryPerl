...
sub seekBoletos{
    my @datos = @_;
    my $sql = "SELECT COMPROBANTE, FACTURADO, ID_REFERENCIA, TARIFA, TOTAL_IVA, ID_BOLETO, TRAB_ID, ID_TERMINAL ".
	      "FROM ENERO_2022 E ".
...
my $file = $ARGV[0];
if(length($file) == 0){
   print("El  nombre del archivo como parametros\n");
   exit;
}
...
while(<FH>){
   ...
   print("$valores[0], $valores[1], $cve\n");
   @comprobante = &seekBoletos( $valores[0], $valores[1], $cve) ;
   if(length($comprobante[2]) == 0){
   	print("$valores[0],$valores[1],$cve, $comprobante[3]\n");
	&updateBoletoParaFacturar($valores[0], $valores[1], $cve, 2022, 'MAYO');
   }else{
	  ...
	  &updateBoletoParaFacturar($valores[0], $valores[1], $cve, 2022, 'CLIENTE_MAYO');
        }
   }
}
close;
