...
uses
  ...
  ActnList, PlatformDefaultStyleActnCtrls, ComCtrls, System.Actions;
...
begin
   ...
   if Trim(edtIDterminal.Text) <> '' then
      if uterminales.ExisteTerminal(Trim(edtIDterminal.Text)) then
         ...
         edtDescripcion.Text, cbTipo.CurrentID, edtVentanillas.Text, cbTipoVenta.CurrentID, lsEmpresa.CurrentID, edtNombreCliente.Text) then
            ShowMessage(_ACTUALIZACION_EXITOSA)
         else
            ShowMessage(_OPERACION_FALLIDA)
      else
         if uTerminales.InsertaTerminal(Trim(edtIDterminal.Text),
                                         edtDescripcion.Text,
                                         cbTipo.CurrentID,
                                         edtVentanillas.Text,
                                         cbTipoVenta.CurrentID ,
                                         lsEmpresa.CurrentID ,
    ...
    begin
      ...
      begin
         ...
         begin
           ...
           lsEmpresa.setID(uTerminales.QueryTerminales.FieldByName('EMPRESA').AsString);
           edtNombreCliente.Text :=  uTerminales.QueryTerminales.FieldByName('NOMBRE_AL_CLIENTE').AsString;
         end;
      end;
         PasaSiguiente;
    end;
