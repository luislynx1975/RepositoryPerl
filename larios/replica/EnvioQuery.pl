...
my $host = "192.168.1.30";
my $password = "PdvMysql";
my $_SERVER = undef;
my $_REMOTO = undef;
...
sub querys($){
	...
	$qry = "SELECT ID_TERMINAL, SENTENCIA, CONSECUTIVO FROM PDV_T_QUERY WHERE ID_TERMINAL = ? ORDER BY CONSECUTIVO  LIMIT 1";
	...
	while( @outs = $ltr->fetchrow_array() ){
		...
		if( ($outs[1] =~ /^INSERT/) ){
		    ...
		    if( $@ =~ m/Duplicate/){
			$rows_affected = 1;
			$query = "INSERT INTO ERROR_TABLA(HORA_ERROR, ERROR, INSTRUCCION ) ".
                                 "VALUES(NOW(), ?, ? )  ";
			$lerror = $_SERVER->prepare($query);
			$lerror->bind_param(1, $outs[0] . '-Duplicate');
...
#		print("rows affected : $rows_affeted \n");
