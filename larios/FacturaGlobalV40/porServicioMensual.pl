...
#Generacion de facturas globales mensuales
...
sub getStatusGlobal{
    ...
    my $sql = "SELECT  DISTINCT(e.ID_TERMINAL), e.TIPOSERVICIO, COUNT(*) ".
	      ...
	      "WHERE g.ID_GRUPO = 'PUL' AND TARIFA > 0 AND ESTATUS = 'V' AND ".
              " e.FECHA_HORA_BOLETO BETWEEN ? AND ? ".
	      "GROUP by 2, 1  ".
	      "UNION  ".
    ...
    $db_sstatus->bind_param(1, $datos[1]);
    $db_sstatus->bind_param(2, $datos[2]);
    $db_sstatus->bind_param(3, $datos[1]);
    $db_sstatus->bind_param(4, $datos[2]);
    $db_status->execute();
...
sub getBoletoServicio5{
    ...
    if($datos[5] != 5){
       $sql = "SELECT COUNT(*) ".
	      ...
	      "AND e.TIPOSERVICIO = ? AND id_terminal = ? AND TARIFA > 0";
       $db_boleto = $S_BOLETO->prepare( $sql );
    ...
    if($datos[5] == 5){
	  ...
	  $iva = 16;
       ...
       $sql = "SELECT COUNT(*) ".
	      "FROM $tableNombre e ".
	      "WHERE  e.FECHA_HORA_BOLETO BETWEEN ? AND ?  ".
	      "AND e.TIPOSERVICIO = ? AND id_terminal = ? AND e.IVA = ? AND TARIFA > 0 ";
       $db_boleto = $S_BOLETO->prepare( $sql );
       $db_boleto->bind_param(1, $datos[1]);
       $db_boleto->bind_param(2, $datos[2]);
       $db_boleto->bind_param(3, $datos[5]);
...
for my $idx (0..@aGblFact -1){
	...
	$bol5ConIva = &getBoletoServicio5( $aGblFact[$idx][0], $inicioMes . ' 00:00:00',
					   $finMes . ' 23:59:59', 0, $a_fecha[0], $aGblFact[$idx][1] );	
     }else{
	$bol5ConIva = &getBoletoServicio5( $aGblFact[$idx][0], $inicioMes . ' 00:00:00',
					   $finMes . ' 23:59:59', 0, $a_fecha[0], $aGblFact[$idx][1] );	
     }
	...
	$bol5ConIva = &getBoletoServicio5( $aGblFact[$idx][0], $inicioMes . ' 00:00:00',
					   $finMes . ' 23:59:59', 1, $a_fecha[0], $aGblFact[$idx][1] );	
	if($bol5ConIva > 0){
