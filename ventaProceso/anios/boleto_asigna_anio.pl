...
sub getFechasBoleto{
	...
	my @a_fechas = undef;
	my %h_fechas = ();
#$query = "SELECT FECHA FROM PDV_T_BOLETO  WHERE COPIADO = 0 AND FECHA <> '0000-00-00' LIMIT 100";
	$query = "SELECT FECHA FROM PDV_T_BOLETO  WHERE COPIADO = 0 LIMIT 100";
...
sub getBoletosVenta{
	...
	my $query = "SELECT ID_BOLETO, ID_TERMINAL, TRAB_ID, ESTATUS, ORIGEN, DESTINO, TARIFA, ID_FORMA_PAGO, ID_TAQUILLA, TIPO_TARIFA, FECHA_HORA_BOLETO, ".
		    ...
		    "TC, IVA, TOTAL_IVA, FACTURADO, ID_REFERENCIA, COPIADO ".
                    "FROM PDV_T_BOLETO B ".
                    "WHERE FECHA = ?  AND COPIADO = 0 LIMIT 70";
	my $sth = $SERVER->prepare( $query );
