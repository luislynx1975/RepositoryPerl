...
sub getFechasBoleto{
	...
	my $sth = undef;
	my @a_fechas = undef;
	$query = "SELECT DISTINCT( FECHA ) FROM PDV_T_BOLETO WHERE FECHA IS NOT NULL AND FECHA <> '0000-00-00' AND ".
	         "FECHA BETWEEN '2015-01-01' AND '2015-12-31'  ORDER BY 1 DESC";	
...
sub createTableAnio{
	...
	my $query = "CREATE TABLE $nombre_tabla( ".
		    ...
		    "INDEX IDX_PDV_T_BOLETO_3 (FECHA) USING HASH, ".
...
print "saliendo de existenTablas\n";
}
...
sub getBoletosVenta{
	...
	my $query = "SELECT ID_BOLETO, ID_TERMINAL, TRAB_ID, ESTATUS, ORIGEN, DESTINO, TARIFA, ID_FORMA_PAGO, ID_TAQUILLA, TIPO_TARIFA, FECHA_HORA_BOLETO, ".
		    ...
		    "TC, IVA, TOTAL_IVA, FACTURADO, ID_REFERENCIA, COPIADO ".
                    "FROM PDV_T_BOLETO B ".
                    "WHERE FECHA = ?  AND COPIADO = 0 ";
	my $sth = $SERVER->prepare( $query );
	$sth->bind_param(1, $fecha );
	$sth->execute();
	if($sth->rows > 0){
		while( (my @outs) = $sth->fetchrow_array() ){
			$tabla = $tabla . substr($outs[10],0,4);
