...
"where fechas between ( SELECT DATE_SUB(CURRENT_DATE(),INTERVAL 3000 DAY) ) and ( CURRENT_DATE() ) ORDER BY 1";
#"where fechas between ( SELECT DATE_SUB(CURRENT_DATE(),INTERVAL 100 DAY) ) and ( CURRENT_DATE() ) ORDER BY 1";
...
sub createTableAnio{
	...
	my $query = "CREATE TABLE $nombre_tabla( ".
		    ...
		    "NOMBRE_PASAJERO VARCHAR(100) NULL DEFAULT NULL, ".
		    ...
		    "CVE_COMPROBANTE_NOTA_CREDITO INT(11) NULL DEFAULT 0,".
		    ...
		    "INDEX COMPROBANTE (COMPROBANTE) USING BTREE, ".
		    "INDEX REFERENCIA (ID_REFERENCIA) USING BTREE ".
	    	    ") ".
...
sub getBoletosVenta{
	...
	my $query = "SELECT ID_BOLETO, ID_TERMINAL, TRAB_ID, ESTATUS, ORIGEN, DESTINO, TARIFA, ID_FORMA_PAGO, ID_TAQUILLA, TIPO_TARIFA, FECHA_HORA_BOLETO, ".
		    ...
		    "FECHA_BOLETO, HORA_BOLETO, TARIFA_REAL, COMPLEMENTO_1, HORA_CORRIDA ".
                    "FROM PDV_T_BOLETO B ".
                    "WHERE COPIADO = 0 AND FECHA_HORA_BOLETO BETWEEN  ? AND ? ORDER BY FECHA_HORA_BOLETO LIMIT 100";
	my $sth = $SERVER->prepare( $query );
	...
	if($sth->rows > 0){
		while( (my @outs) = $sth->fetchrow_array() ){
			...
			my $s_inserta = "INSERT INTO $tabla( ID_BOLETO, ID_TERMINAL, TRAB_ID, ESTATUS, ORIGEN, DESTINO, TARIFA, ID_FORMA_PAGO, ID_TAQUILLA, TIPO_TARIFA, FECHA_HORA_BOLETO, ".
		    					    "ID_CORRIDA, FECHA, NOMBRE_PASAJERO, NO_ASIENTO, ID_OCUPANTE, ESTATUS_PROCESADO, ID_RUTA, TIPOSERVICIO, ID_PAGO_PINPAD_BANAMEX, ".
		    					    "TC, IVA, TOTAL_IVA, FACTURADO, ID_REFERENCIA, COPIADO, ID_FORMA_PAGO_SUB, ".
		    					    "FECHA_BOLETO, HORA_BOLETO, TARIFA_REAL, COMPLEMENTO_1, HORA_CORRIDA) ".
					"VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )";
...
print "Paso inserta en la tabla 6   $outs[26] $outs[0]  $outs[1] $outs[2] $tabla $outs[10]\n";
