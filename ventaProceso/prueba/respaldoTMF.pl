...
my @a_fechas = ();
my @a_terminales = ();
#my @a_tablas = ('PDV_T_CORRIDA','PDV_T_CORRIDA_D','PDV_T_GUIA','PDV_T_CORTE','PDV_T_BOLETO_CANCELADO'  );
my @a_tablas = ('PDV_T_BOLETO','PDV_T_CORRIDA','PDV_T_CORRIDA_D','PDV_T_GUIA','PDV_T_CORTE','PDV_T_BOLETO_CANCELADO'  );
...
sub getQueryTabla(){
	...
	switch($tabla){
	   ...
	   case "PDV_T_GUIA" {
		#$query_where = " CAST(FECHA_HORA AS DATE) = '$fecha' OR G.FECHA = CURRENT_DATE() ";
	   ...
	   case "PDV_T_CORRIDA" {
		...
		$query_where = "SELECT C.ID_CORRIDA,C.FECHA,C.HORA,ID_TIPO_AUTOBUS,TIPOSERVICIO,C.ID_RUTA,NO_BUS,TIPO_CORRIDA, C.CREADO_POR ".
			       ...
			       "WHERE C.FECHA = '$fecha' AND D.ID_TERMINAL = '$terminal' OR C.FECHA = CURRENT_DATE() ";
	   }
...
sub setDatosTabla(){
	...
	if($li_adm > 0){
	   ...
	   $stg->execute();
	   $stg->finish();
	   print "Proceso $s_tabla del dia $s_fecha de la terminal $s_terminal\n";
	}
}
