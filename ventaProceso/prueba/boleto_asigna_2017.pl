...
#		 "FROM PDV_T_BOLETO B ".
#		 "WHERE FECHA BETWEEN CURRENT_DATE() AND (SELECT MAX(FECHA) ".
...
#	print "existe el arreglo "  if defined(@a_fecha_boleto);
	if (!defined $a_fecha_boleto[0]){
#		print "el arreglo no esta definido \n";
		$query = "SELECT DISTINCT(CAST(FECHA_HORA_BOLETO AS DATE)) ".
			 "FROM PDV_T_BOLETO ".
			 "WHERE COPIADO IN (3) ".
			 "LIMIT 1";
#print $query . "\n";
...
sub getBoletosVenta{
	...
	my $query = "SELECT ID_BOLETO, ID_TERMINAL, TRAB_ID, ESTATUS, ORIGEN, DESTINO, TARIFA, ID_FORMA_PAGO, ID_TAQUILLA, TIPO_TARIFA, FECHA_HORA_BOLETO, ".
		    ...
		    "TC, IVA, TOTAL_IVA, FACTURADO, ID_REFERENCIA, COPIADO ".
                    "FROM PDV_T_BOLETO B ".
                    "WHERE COPIADO IN (3) AND FECHA = ? ";
                    #"WHERE COPIADO IN (1,2) AND FECHA = ? LIMIT 5000";
	my $sth = $SERVER->prepare( $query );
...
print "..... " .$fecha . " \n" . $query . "\n";
	...
	if($sth->rows > 0){
		while( (my @outs) = $sth->fetchrow_array() ){
			...
			my $s_tc_busca = "SELECT COUNT(*) FROM $tabla WHERE TC = ?";
			my $stb = $SERVER->prepare($s_tc_busca);
			$stb->bind_param(1,$outs[20]);
			$stb->execute();
#print $stb->rows . " valor $outs[0] $outs[1] $outs[2] $outs[3] $outs[4] $outs[20] $tabla\n";
			if($stb->rows > 0 ){
