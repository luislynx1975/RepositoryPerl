...
my @a_terminales = ();
my $cuantos = 0;
#my @a_tablas = ('PDV_T_BOLETO','PDV_T_CORRIDA','PDV_T_CORRIDA_D','PDV_T_GUIA','PDV_T_CORTE',
#		'PDV_T_BOLETO_CANCELADO','PDV_T_RECOLECCION');
...
sub getTerminales{
	...
	my $sth = $_SERVER->prepare("SELECT T.ID_TERMINAL, T.IPv4, T.BD_BASEDATOS, T.BD_USUARIO, T.BD_PASSWORD, T.ESTATUS ".
				    ...
				    "WHERE T.ESTATUS = 'A' AND ID_TERMINAL = 'SD1'  ORDER BY 1 ");
	$sth->execute();
...
sub getFechasSistema{
	...
	my $sth = $_SERVER->prepare("SELECT DISTINCT(FECHA) FROM PDV_T_BOLETO ".
				    "WHERE FECHA < CURRENT_DATE() AND FECHA > '2018-01-01' AND ".
