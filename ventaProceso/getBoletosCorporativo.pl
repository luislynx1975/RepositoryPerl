...
my @a_terminales = ();
my $cuantos = 0;
my @a_tablas = ('PDV_T_CORRIDA','PDV_T_GUIA','PDV_T_CORTE','PDV_T_BOLETO_CANCELADO','PDV_T_RECOLECCION');
#my @a_tablas = ('PDV_T_BOLETO');
...
sub getFechasSistema{
	...
	my $sth = $_SERVER->prepare("SELECT DISTINCT(FECHA) FROM PDV_T_BOLETO ".
				    "WHERE FECHA < CURRENT_DATE() AND FECHA > '2019-01-01' AND ".
...
######################################################################
#Extraemos como esta contruida la tabla en la base de datos remota
sub DDgetDescripcionTabla(){
	my ($tabla, $servidor) = @_;
	my $_MYSQL = &conectaServer('information_schema', $servidor, $user, $pass);
#	my $query = "SELECT COLUMN_NAME FROM COLUMNS WHERE TABLE_SCHEMA = 'CORPORATIVO' AND TABLE_NAME = '$tabla'";
	my $query = " '$tabla'";
	my $li_indice = 0;
	my @describe;
	my $sth = $_REMOTO->prepare( $query );
	#my $sth = $_MYSQL->prepare( $query );
	$sth->execute();
	while(my @outs = $sth->fetchrow_array()) {
	    $describe[$li_indice] = $outs[0];
	    $li_indice++;
...
sub getTotalRegistrosTabla(){
	...
	my $query = "SELECT COUNT(*) FROM PDV_T_BOLETO ".
		    "WHERE ID_TERMINAL = '$terminal' AND FECHA_HORA_BOLETO BETWEEN '$fecha 00:00:00' AND '$fecha 23:59:59' AND ID_TERMINAL = '$terminal'";
	my $stl = $_SERVER->prepare( $query );
	$stl->execute();
#print $query;
	my @outs = $stl->fetchrow_array();
...
#               if(@{$a_fecha_tablas[$x]}){
		if(@a_adm_tabla){
		    ...
		    $li_busca = 0;
		    foreach my $kk (@a_adm_tabla){
    		    #    $a_adm_tabla[$li_busca] = $a_adm_tabla[$kk];
			$li_busca++;
		    }
		    my @a_campos = &getDescripcionTabla(uc($a_adm_tabla[1]), $remoto_host)  ;
print $a_adm_tabla[1] ."\n";
		    my $query_consulta = &getQueryTabla(uc($a_adm_tabla[1]), $a_adm_tabla[0], @{$hash{$k}}[0], @a_campos ) ;
print($query_consulta. "\n");
		    my @a_remoto = &getDatosTabla( $query_consulta, $a_adm_tabla[0], &getDescripcionTabla(uc($a_adm_tabla[1]), $remoto_host)  );
