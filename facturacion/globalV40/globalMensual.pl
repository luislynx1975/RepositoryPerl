...
my $d_bFACTURA = "FACTURACION";
my $d_hFACTURA = "192.168.1.26"; #P R O D U C T I V O
################my $d_hFACTURA = "192.168.1.15"; # D E S A R R O L L O
...
sub getTerminalGlobal{
    ...
    my $sql = "SELECT DISTINCT(ID_TERMINAL) FROM FAC_C_TAQUILLA_GLOBAL ".
	      "WHERE ID_TERMINAL NOT IN ('SD1','SD2') AND (RFC = 'APC580909L82','TCC591104A74')";
#	      "WHERE ID_TERMINAL NOT IN ('SD1','SD2') AND (RFC = 'APC580909L82')";
...
sub getServicioTaquilla{
    ...
    my $sql = "SELECT DISTINCT(TIPOSERVICIO) ".
              "FROM $name_table  ".
              "WHERE CAST(FECHA_HORA_BOLETO AS DATE) between ? AND ? AND ID_TERMINAL = ? AND ESTATUS = 'V' AND FACTURADO = ? ";
    ...
    $db_corpo->bind_param(4, $facturar);
    $db_corpo->execute();
...
sub getFormaPagoBoleto{
    ...
    my $sql = "SELECT B.ID_FORMA_PAGO, COUNT(B.ID_FORMA_PAGO)AS TOTAL ".
              ...
              " TIPOSERVICIO = ? AND B.FACTURADO = ? GROUP BY ID_FORMA_PAGO ORDER BY 2 DESC LIMIT 1";
...
sub getComprobante{
    ...
    $db_factura->bind_param(14, '019969G');  #usuario
    ...
    $db_factura->bind_param(27, '04');      #periodicidad
    $db_factura->bind_param(28, $datos[4]);      #mes
    $db_factura->bind_param(29, $datos[3]);      #anio
    $db_factura->bind_param(30, 1);      #region
    $db_factura->bind_param(31, '01');      #exportacion
    $db_factura->execute();

    my $query = "SELECT \@outClave, \@outEstatus ";
    $db_factura =$S_FACTURA->prepare( $query );
    $db_factura->execute();
    my @datas = $db_factura->fetchrow_array();
    $out = $datas[0];    
    $db_factura->finish();
    return $out;
}
...
sub getBoletosAnio{
    ...
    my $sql = "SELECT B.ID_BOLETO, B.ID_TERMINAL, B.TRAB_ID, ORIGEN, DESTINO, FECHA_HORA_BOLETO, ".
              "TC, IVA * 1/100, TOTAL_IVA, TARIFA, (TARIFA - TOTAL_IVA) AS SUBTOTAL ".
              "FROM $tableNombre B ".
              "WHERE FECHA_HORA_BOLETO  BETWEEN ? AND ? AND ESTATUS = 'V' AND B.ID_TERMINAL = ? AND ".
              "TARIFA > 0 AND TIPOSERVICIO = ? AND FACTURADO = ?";
    my $db_boleto = $S_BOLETO->prepare( $sql );
    $db_boleto->bind_param(1, $f_inicial );
    $db_boleto->bind_param(2, $f_final);
    $db_boleto->bind_param(3, $valor[2]);
    ...
    $db_boleto->bind_param(5, $facturar);
    $db_boleto->execute();
    while(my @datos = $db_boleto->fetchrow_array()){#insertamos en la tabla de factura los conceptos camibiar por la nueva funcion
...
######@aDias = ('2021-02-01','2021-01-01','2021-01-31');
...
my $contador = 1;
print("$dayMes   ........   $diaActual    $dayLastMes    $lastDayMes\n");
if ($dayMes =~ $diaActual){
...
print("$terminal    $servicio\n");
	  if ($servicio > 0) {
	        ...
	        &setFechaStatus($dayLastMes, $terminal, 9999, $servicio, $mesGlobal, $anioGlobal[0]);
                my $idComprobante = &getComprobante($formaPago, $terminal, $servicio, $anioGlobal[0], $anioGlobal[1]);
