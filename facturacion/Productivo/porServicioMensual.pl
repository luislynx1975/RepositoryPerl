...
###################Insertamos en la tabla de translado
	...
	#####Aactualizamos la tabla de boleto con el comprobante y tipo_factura = G
        if($valor[1] > 0){
	    my $sql = "UPDATE $tableNombre SET COMPROBANTE = ?, TIPO_FACTURA = ?, FACTURADO = 1,  COMPLEMENTO_2 = 'FACTURADO MES'  ".
		      "WHERE ID_BOLETO = ? AND ID_TERMINAL = ? AND TRAB_ID = ?";
	    my $db_corpo = $S_BOLETO->prepare( $sql );
    	    $db_corpo->bind_param(1, $comprobanteID);
...
sub getBoletosAnio{#año, fecha, terminal, comprobante, servicio, extraordinario fechaFin
    ...
    if( $valor[4] != 5){
    	$sql = "SELECT B.ID_BOLETO, B.ID_TERMINAL, B.TRAB_ID, ORIGEN, DESTINO, FECHA_HORA_BOLETO, ".
	      	...
	      	" FECHA_HORA_BOLETO BETWEEN ? AND ? AND FACTURADO = 0 "; 
    	$db_boleto = $S_BOLETO->prepare( $sql );
    	$db_boleto->bind_param(1, $valor[2]);
    	$db_boleto->bind_param(2, $valor[4]);
    	$db_boleto->bind_param(3, $f_inicial);
    	$db_boleto->bind_param(4, $f_final);
        $db_boleto->execute();
    }
	...
	if($valor[5] == 0){
    	...
    	$sql = "SELECT B.ID_BOLETO, B.ID_TERMINAL, B.TRAB_ID, ORIGEN, DESTINO, FECHA_HORA_BOLETO, ".
	       ...
	       "FROM $tableNombre B ".
	       "WHERE ESTATUS = 'V' AND B.ID_TERMINAL = ? AND TARIFA > 0 AND TIPOSERVICIO = ? AND  ".
	       "  IVA = ? AND FECHA_HORA_BOLETO BETWEEN ? AND ?  AND FACTURADO = 0 ";
    	$db_boleto = $S_BOLETO->prepare( $sql );
    	$db_boleto->bind_param(1, $valor[2]);
    	$db_boleto->bind_param(2, $valor[4]);
    	$db_boleto->bind_param(3, $iva);
    	$db_boleto->bind_param(4, $f_inicial);
    	$db_boleto->bind_param(5, $f_final);
        $db_boleto->execute();
    }
    while(my @datos = $db_boleto->fetchrow_array()){#insertamos en la tabla de factura los conceptos
	$totalBoletos++; 
...
sub getComprobanteV40{# formaPago,terminal,TipoServicio,ExtraOdinario
    ...
    $db_factura->bind_param(24,  'MENSUAL');       #referencia
...
print("Error en el comprobante $datos[1]   $datos[0]\n");
    return $datos[0];
    $db_factura->finish();
}
...
sub getStatusGlobal{
    ...
    my @out = ();
    my $idx = 0;
    my $tableNombre = $tableName . $datos[0];
    my $sql = "SELECT DISTINCT(e.ID_TERMINAL), e.TIPOSERVICIO, COUNT(*), SUM(tarifa)AS total ".
	      "FROM $tableNombre e  INNER JOIN PDV_C_GRUPO_SERVICIOS g ON e.ID_TERMINAL = g.ID_TERMINAL ".
	      "WHERE TARIFA > 0 AND ESTATUS = 'V' AND ".
              " e.FECHA_HORA_BOLETO BETWEEN ? AND ? AND FACTURADO = 0 ".
	      "GROUP by 2, 1  ".
 	      "Having total > 0 ".
	      "UNION  ".
	      "SELECT DISTINCT(e.ID_TERMINAL), e.TIPOSERVICIO, COUNT(*), SUM(tarifa)AS total ".
	      "FROM $tableNombre e  ". 
	      "WHERE e.ID_TERMINAL IN ('WEB','MOV') AND TARIFA > 0 AND ESTATUS = 'V' AND ".
              " e.FECHA_HORA_BOLETO BETWEEN ? AND ? AND FACTURADO = 0 ".
	      "GROUP by 2, 1 ".
 	      "Having total > 0 ".
	      "ORDER BY 1"; 
    ...
    $db_status->bind_param(1, $datos[1]);
    $db_status->bind_param(2, $datos[2]);
    $db_status->bind_param(3, $datos[1]);
    $db_status->bind_param(4, $datos[2]);
    $db_status->execute();
...
sub getBoletoServicio5{
    ...
    if($datos[5] != 5){
       $sql = "SELECT COUNT(*) ".
	      ...
	      "AND e.TIPOSERVICIO = ? AND id_terminal = ? AND TARIFA > 0 AND FACTURADO = 0 ";
       $db_boleto = $S_BOLETO->prepare( $sql );
    ...
    if($datos[5] == 5){
	  ...
	  $iva = 16;
       ...
       $sql = "SELECT COUNT(*) ".
	      "FROM $tableNombre e ".
	      "WHERE  e.FECHA_HORA_BOLETO BETWEEN ? AND ?  ".
	      "AND e.TIPOSERVICIO = ? AND id_terminal = ? AND e.IVA = ? AND TARIFA > 0 AND FACTURADO = 0 ";
       $db_boleto = $S_BOLETO->prepare( $sql );
       $db_boleto->bind_param(1, $datos[1]);
       $db_boleto->bind_param(2, $datos[2]);
       $db_boleto->bind_param(3, $datos[5]);
...
sub insertTmpBoleto7000{
    ...
    $sql = "INSERT INTO TMP_BOLETO ".
	   "SELECT * ".
	   "FROM $tableNombre e  ".
	   "WHERE e.TIPOSERVICIO = ? AND id_terminal = ? AND FACTURADO = 0 AND ESTATUS = 'V' AND ".
           " FECHA_HORA_BOLETO BETWEEN ? AND ? ".
	   " LIMIT 7000";
    $db_corpo = $S_BOLETO->prepare( $sql );
...
sub cambiaMesComplemento{# año terminal servicio extraordinario
    ...
    my $sql = "UPDATE $tableNombre e INNER JOIN TMP_BOLETO t ON t.ID_BOLETO = e.ID_BOLETO AND t.ID_TERMINAL = e.ID_TERMINAL AND t.TRAB_ID = e.TRAB_ID ".
	      "SET e.COMPLEMENTO_2 = 'FACTURADO GLOBAL'";
    my $db_corpo = $S_BOLETO->prepare( $sql );
    $db_corpo->execute();
}
...
sub getDiaForMes{
    ...
    $out[0] = $year . '-' . $mes . '-' . '01';
    my $ultimo = DateTime->last_day_of_month({
						year => $year,
                                                month => $mes
					     })->ymd('-');
    $out[1] = $ultimo;
...
my @dias = &getDiaForMes();
my $diaActual = &getDiaActual();
print("fin Mes :-$dias[1]-   Actual :-$diaActual-\n");
if($dias[1] ne $diaActual){
   print("El dia de hoy: $diaActual no es igual al ultimo dia del mes $dias[1]\n");
   exit;
}
my $diaAnteriorMes = &getDiaAnteriorAlMes();
my $inicioMes = $dias[0];
my $finMes = $dias[1];
my $bol5ConIva = 0;
...
my @a_fecha = split(/-/,$inicioMes);
my @aGblFact = getStatusGlobal($a_fecha[0], $inicioMes . ' 00:00:00', 
					    $finMes . ' 23:59:59');
my @global = undef;
for my $idx (0..@aGblFact -1){
     if($aGblFact[$idx][1] != 5){
	$bol5ConIva = &getBoletoServicio5( $aGblFact[$idx][0], $inicioMes . ' 00:00:00',
					   $diaAnteriorMes . ' 23:59:59', 0, $a_fecha[0], $aGblFact[$idx][1] );	
print("aqui entro\n");
     }else{
	$bol5ConIva = &getBoletoServicio5( $aGblFact[$idx][0], $inicioMes . ' 00:00:00',
					   $diaAnteriorMes . ' 23:59:59', 0, $a_fecha[0], $aGblFact[$idx][1] );	
     ...
     if($bol5ConIva > 0){
        &setMesMensual($aGblFact[$idx][0], $inicioMes, $finMes , $aGblFact[$idx][1], 0, $bol5ConIva);
        print("p.....$aGblFact[$idx][0], $inicioMes, $finMes, $aGblFact[$idx][1], 0, $bol5ConIva\n");
     }
     if($aGblFact[$idx][1] == 5){
	$bol5ConIva = &getBoletoServicio5( $aGblFact[$idx][0], $inicioMes . ' 00:00:00',
					   $diaAnteriorMes . ' 23:59:59', 1, $a_fecha[0], $aGblFact[$idx][1] );	
	if($bol5ConIva > 0){
            &setMesMensual($aGblFact[$idx][0], $inicioMes, $finMes, $aGblFact[$idx][1], 1, $bol5ConIva);
            print("..$aGblFact[$idx][0], $inicioMes, $finMes, $aGblFact[$idx][1], 1, $bol5ConIva\n");
	}
     }
}#fin for
my @global = &getGlobalMensual($inicioMes, $finMes);
my $numFactura = 1;
for my $idx (0..@global -1){
   $global[$idx][2] = $diaAnteriorMes;
   if($global[$idx][4] > $NUM_ROWS ){
	...
	print("Numero $numFactura  $x \n");
	&insertTmpBoleto7000($global[$idx][1] . ' 00:00:00',
			     $global[$idx][2] . ' 23:59:59', 
                             $global[$idx][3], 
			     $global[$idx][0], 
			     $global[$idx][6] );
	...
	print("Pause\n");
        ...
        &getBoletosAnio($a_fecha[0], 
			$global[$idx][1], 
			$global[$idx][0], 
			$idComprobante, 
			$global[$idx][3], 
			$global[$idx][5], 
			$global[$idx][2]);#año, fecha, terminal, comprobante, servicio, extraordinario fechaFinal
     	&setTimbra($idComprobante);
print("Aqui entro   $idComprobante\n");
print("$global[$idx][1], $global[$idx][0],   $idComprobante, $global[$idx][3], $global[$idx][5]\n");
