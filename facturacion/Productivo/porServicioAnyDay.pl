...
sub getComprobanteV40{# formaPago,terminal,TipoServicio,ExtraOdinario
    ...
    my $ls_servicio;
    if( $datos[2] < 10){
        $ls_servicio = '0'  . $datos[2];
    }else{
        $ls_servicio = $datos[2];
    }

    my $sql = "CALL pCOMPROBANTE_INSERTA_V40_A(?, ?, ?, ?, ?, ". #5
                                            "?, ?, ?, ?, ?, ". #10
                                            "?, ?, ?, ?, ?, ". #15
                                            "?, ?, ?, ?, ?, ". #20
                                            "?, ?, ?, ?, ?, ". #25
...
#					    "?, \@outClave, \@outEstatus ) ";
    ...
    $db_factura->bind_param(32, $_UBICACION);      #procedencia
    $db_factura->bind_param(33, $ls_servicio );      #servicio
    $db_factura->bind_param(34, $_USR_FACT);      #user
    $db_factura->execute();
...
sub getStatusGlobal{
    ...
    my $sql = "SELECT ID_TERMINAL, FECHA, SERVICIO, EXTRAORDINARIA ".
	      ...
	      "WHERE FECHA <= SUBDATE(CURRENT_DATE(),INTERVAL 1 DAY) AND STATUS = 0";
...
#        print("$aGblFact[$idx][0] - $aGblFact[$idx][1] - $aGblFact[$idx][2] - $aGblFact[$idx][3] \n" );
     ...
     if($idComprobante > 0 ){
        my @a_fecha = split('-', $aGblFact[$idx][1]);
