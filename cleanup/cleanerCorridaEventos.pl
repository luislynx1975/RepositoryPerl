...
sub getBoletoTrabId{
    ...
    $d_cves->bind_param(1, $terminal);
    $d_cves->execute();
    while( my @vals = $d_cves->fetchrow_array() ){
	print("valores $vals[0] \n");
	$datos[$idx] = $vals[0];
	$idx++;
    }
    return @datos;
...
sub backUpTickets{
     ...
     $d_ticket->bind_param(2, $dias); 
     $d_ticket->bind_param(3, $terminal); 
     $d_ticket->execute();
     while( my @outs = $d_ticket->fetchrow_array() ){
	my $query = "INSERT INTO PDV_T_BOLETO_BORRADOS(ID_BOLETO, ID_TERMINAL, TRAB_ID, ESTATUS, ORIGEN, DESTINO, TARIFA, ID_FORMA_PAGO, ID_TAQUILLA, TIPO_TARIFA, FECHA_HORA_BOLETO, ID_CORRIDA, FECHA, ".
		"NOMBRE_PASAJERO, NO_ASIENTO, id_ocupante, ESTATUS_PROCESADO, ID_RUTA, TIPOSERVICIO, ID_PAGO_PINPAD_BANAMEX,  ".
		"TC, IVA, TOTAL_IVA, ID_FORMA_PAGO_SUB, FECHA_BOLETO, HORA_BOLETO, TARIFA_REAL, HORA_CORRIDA) ".
		"VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?  )";
...
sub deleteTickets{
     ...
     $query = "DELETE ".
	      "FROM PDV_T_ASIENTO ".
              "WHERE CAST(FECHA_HORA_CORRIDA AS DATE) = ?";
...
sub getDaysRange(){
	...
	my $qry = "select * from ".
		  ...
		  " (select 0 i union select 1 union select 2 union select 3 union select 4 union select 5 union select 6 union select 7 union select 8 union select 9) t3, ".
		  " (select 0 i union select 1 union select 2 union select 3 union select 4 union select 5 union select 6 union select 7 union select 8 union select 9) t4) v ".
		  " where fechas BETWEEN (SELECT MIN(D.FECHA) FROM PDV_T_CORRIDA D) and ( SUBDATE(CURRENT_DATE(), INTERVAL 100 DAY) ) ORDER BY 1 ";
...
sub getDaysRange30(){
	...
	my $qry = "select * from ".
		  ...
		  " (select 0 i union select 1 union select 2 union select 3 union select 4 union select 5 union select 6 union select 7 union select 8 union select 9) t3, ".
		  " (select 0 i union select 1 union select 2 union select 3 union select 4 union select 5 union select 6 union select 7 union select 8 union select 9) t4) v ".
		  " where fechas BETWEEN (SELECT MIN(D.FECHA) FROM PDV_T_CORRIDA D) and ( SUBDATE(CURRENT_DATE(), INTERVAL 30 DAY) ) ORDER BY 1 ";
		  #" where fechas BETWEEN '2010-01-01' and ( SUBDATE(CURRENT_DATE(), INTERVAL 30 DAY) ) ORDER BY 1 ";
	my $d_days = $R_SERVER->prepare( $qry );
	$d_days->execute();
	while( my @data = $d_days->fetchrow_array()) {
	   $outDays[$idx] = $data[0];
...
sub deleteCorridas(){
   my @a_param = @_;
print("Borrando corridas : $a_param[0]   \n");
   my $qry = "DELETE FROM PDV_T_CORRIDA_D WHERE FECHA = ? AND ID_TERMINAL = ?";
   my $borra = $R_SERVER->prepare( $qry );
   $borra->bind_param(1, $a_param[0]);
   $borra->bind_param(2, $a_param[1]);
   $borra->execute();

   $qry = "DELETE FROM PDV_T_CORRIDA WHERE FECHA = ?";
   $borra = $R_SERVER->prepare( $qry );
   $borra->bind_param(1, $a_param[0]);
   $borra->execute();

   $qry = "DELETE FROM PDV_T_CORRIDA_OCUPANTE WHERE FECHA = ?";
   $borra = $R_SERVER->prepare( $qry );
   $borra->bind_param(1, $a_param[0]);
   $borra->execute();
}
...
sub deleteEventos(){
   my $day = shift;
print("Borrando eventos: $day\n");
   my $qry = "DELETE FROM EVENTOS WHERE FECHA_HORA BETWEEN ? AND ? ";
   my $eventos = $R_SERVER->prepare( $qry );
   $eventos->bind_param(1, $day . " 00:00:00"); 
   $eventos->bind_param(2, $day . " 23:59:59"); 
   $eventos->execute();

   $qry = "DELETE FROM PDV_T_ASIENTO WHERE FECHA_HORA_CORRIDA BETWEEN ? AND ? ";
   $eventos = $R_SERVER->prepare( $qry );
   $eventos->bind_param(1, $day . " 00:00:00"); 
   $eventos->bind_param(2, $day . " 23:59:59"); 
   $eventos->execute();
}
...
=pod
No se ejecuta esta parte del codigo esta comentado
